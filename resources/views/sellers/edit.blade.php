...
@section('content')
    ...
    <div class="w-full flex flex-wrap">
        <!-- Sidebar -->
        <x-sidebar.sidebar-seller active="profile" />
        ...
        <main class=" max-w-6xl space-y-7 mt-24 ml-96 w-9/12">
            ...
            <div class="grid grid-cols-6 gap-6 max-w-6xl mx-auto ">
                <!-- Profile Image -->
                <div class="flex flex-col items-center justify-around bg-gray-900 p-4 col-span-2" x-data="{ open:false }">
                    <div
                        class="p-1 bg-gradient-to-br from-green-600 to-blue-500 rounded-full w-36 h-36 grid place-items-center">
                        <img src="{{ asset('uploads/seller/' . $sellerInfo->gambarPenjual) }}" alt="img"
                    ...
                    <div class=" fixed w-full h-full top-0 left-0 flex items-center justify-center z-50" x-show="open">
                        ...
                        <div class=" bg-gray-800 w-11/12 md:max-w-md mx-auto rounded-xl shadow-lg z-50 p-2"
                            ...
                            <form action="{{ route('sellers.update', session('id')) }}" method="POST"
                                enctype="multipart/form-data" class="px-6 py-4">
                                @csrf
                                @method('PATCH')
                                <input type="file" name="image" class="pb-3" accept="image/x-png,image/jpeg">
                                <input type="hidden" name="oldImage" value="{{ $sellerInfo->gambarPenjual }}">
                ...
                <!-- User Information -->
                <div class="bg-gray-900 col-span-4 p-4 space-y-5">
                    <div>
                    ...
                    <div>
                        ...
                        <div x-data="{ open : false }">
                            <div x-show="!open" class="grid grid-cols-10 gap-2">
                                <input type="password" value="{{ $sellerInfo->kataLaluanPenjual }}"
                                    class="bg-transparent outline-none border-none focus:ring-0 col-span-8" readonly>
                                <div class=" col-span-2 rounded bg-gray-700 px-4 py-2 font-bold cursor-pointer flex justify-center items-center"
                                    x-on:click="open = !open">Edit</div>
                            </div>

                            <form action="{{ route('sellers.update', session('id')) }}" method="POST" x-show="open"
                                ...
                                <input type="text" name="password"
                                    class="col-span-8 bg-gray-700 rounded-lg py-3 focus:ring-2 ring-green-600 text-lg"
                                    value="{{ $sellerInfo->kataLaluanPenjual }}">
